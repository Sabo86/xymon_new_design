...
void do_vertical(host_t *head, FILE *output, char *pagepath)
{
	...
	fprintf(output, "\n<div class=\"table-row\">");
	fprintf(output, "			<div class=\"table-cell\">");
	fprintf(output, "				<TABLE class=\"darkTable\">");
	fprintf(output, "					<thead>");
	fprintf(output, "						<tr>");

						
	for (h = head, hostcount = 0; (h); h = h->next, hostcount++) {
		fprintf(output, " <TH class=\"table_th\">");
		fprintf(output, " <A class=\"spalten_link\" HREF=\"%s\"><FONT %s></FONT><B>%s</B></A> </TH>\n", 
...
fprintf(output, "		<div class=\"table-row\">");
fprintf(output, "			<div class=\"TableFoot\">");				
fprintf(output, "				<table style=\"margin-left: auto; margin-right: auto;\">");
fprintf(output, "						<tr>");
fprintf(output, "							<td><24h</td>");
...
fprintf(output, "						</tr>");		
fprintf(output, "						<tr>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/green-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/green.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= Alles OK</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/purple-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/purple.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= keine Statusmeldungen </td>");
fprintf(output, "						</tr>");
fprintf(output, "						<tr>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/red-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/red.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= echtes Problem, Aktion notwendig</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/blue-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/blue.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= Anzeige abgeschaltet (disabled)</td>");
fprintf(output, "						</tr>	");													
fprintf(output, "						<tr>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/yellow-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/yellow.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= Achtung, Grenzwert &uuml;berschritten</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/clear-recent.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/clear.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= keine Daten verfügbar</td>");
fprintf(output, "						</tr>	");			
...
void do_hosts(host_t *head, int sorthosts, char *onlycols, char *exceptcols, FILE *output, FILE *rssoutput, char *grouptitle, int pagetype, char *pagepath)
{
	...
	else fprintf(output, "<A NAME=hosts-blk-%d></A>", hostblkidx);*/
	hostblkidx++;

	if (!grouptitle) grouptitle = "";
	safegrouptitle = stripnonwords(grouptitle);
	/* if (*safegrouptitle != '\0') fprintf(output, "<A NAME=\"group-%s\"></A>", safegrouptitle); */
	...
	if (showemptygroups || groupcols) {
		...
		for (h = head; (h); h = h->next) {
			...
			if (h->pretitle && (pagetype == PAGE_NORMAL)) {
				fprintf(output, "<td colspan=%d align=center valign=middle><br><font %s></font>%s</td></tr>\n", 
			...
			if (rowcount == 0) {
				...
				fprintf(output, "<TH class=\"table_projekt_name\"><FONT %s></FONT> %s </TH>\n", 
				...
				for (gc=groupcols; (gc); gc = gc->next) {
					fprintf(output, " <TH class=\"table_th\"> <A class=\"spalten_link\" HREF=\"%s\"> <FONT %s></FONT><B> %s </B> </A> </TH>\n", 
		...
		fprintf(output, "<TR>\n");
			...
			fprintf(output, "<TD class=\"cellhead\" NOWRAP>\n 		<A class=\"zeilen_link\" NAME=\"%s\">&nbsp;</A>\n", h->hostname);
			if (maxrowsbeforeheading) rowcount = (rowcount + 1) % maxrowsbeforeheading;
			else rowcount++;


/* hier wird der Zeilen kopf gebaut */

			fprintf(output, "&nbsp;&nbsp; <B><FONT color=\"#FFFFE0\"> <A class=\"zeilen_link\" %s", 
				hostnamehtml(h->hostname, 
					     ((pagetype != PAGE_NORMAL) ? hostpage_link(h) : NULL), 
					     usetooltip));
						 fprintf(output, "</A></FONT></B>");
			fprintf(output, "\n</TD>");
			...
			for (gc = groupcols; (gc); gc = gc->next) {
				...
				if (e == NULL) {
					...
					if ((e->color == COL_GREEN) || (e->color == COL_CLEAR)) {
						fprintf(output, "\n		<IMG SRC=\"%s/%s\" ALT=\"%s\" TITLE=\"%s\" HEIGHT=\"%s\" WIDTH=\"%s\" BORDER=0>",
							xymonskin, dotgiffilename(e->color, 0, 1),
...
fprintf(output, "								<td>|</td>");
fprintf(output, "							<td align=center><IMG SRC=\"/xymon/gifs/new/clear.gif\" HEIGHT=\"13\" WIDTH=\"13\" BORDER=0></td>");
fprintf(output, "							<td align=left>= keine Daten verf&uuml;gbar</td>");
fprintf(output, "						</tr>	");	
...
void do_groups(group_t *head, FILE *output, FILE *rssoutput, char *pagepath)
{
	...
	for (g = head; (g); g = g->next) {
		if (g->hosts && g->pretitle) {
			...
			fprintf(output, "  <TR><TD><CENTER><FONT %s></FONT>%s</CENTER></TD></TR>\n", xgetenv("XYMONPAGETITLE"), g->pretitle);
...
void do_page_subpages(FILE *output, xymongen_page_t *subs, char *pagepath)
{
	...
	fprintf(output, "<div class=\"table-row\">\n");
		fprintf(output, "<div class=\"table-cell\">\n");		
		fprintf(output, "<CENTER><TABLE SUMMARY=\"Page Block\" class=\"firstTable\" BORDER=0  >\n");
		...
		for (p = subs; (p); p = p->next) {
			if (p->pretitle) {
				...
				fprintf(output, "<thead>\n<TR>\n		<TH class=\"table_first_name\" COLSPAN=%d>\n<FONT %s>\n", 
						(2*subpagecolumns + (subpagecolumns - 1)), xgetenv("XYMONPAGETITLE"));
				fprintf(output, "   %s\n", p->pretitle);
				fprintf(output, "</FONT></TH>\n</TR>\n");				
				fprintf(output, "	</TH>\n</TR>\n</thead>\n<tbody>");
			}
			...
			fprintf(output, "<TD ALIGN=LEFT class=\"mainpage_font\">&nbsp;<FONT %s>", xgetenv("XYMONPAGEROWFONT"));
			fprintf(output, "<A class=\"zeilen_link\" HREF=\"%s\">", cleanurl(pagelink));
			if (linkurl) {
			...
			fprintf(output, "</FONT></A></TD>\n");

			fprintf(output, "<TD class=\"mainpage_font\" ><CENTER><A HREF=\"%s\">", cleanurl(pagelink));
...
int do_nongreen_page(char *nssidebarfilename, int summarytype, char *filenamebase)
{
	...
	fprintf(output, "<div class=\"table-row\">\n");
	fprintf(output, "<div class=\"table-cell\">\n");
	fprintf(output, "<TABLE style=\"width: 100%; height: 100%;  margin-left: auto; margin-right: auto;\" class=\"darkTable\">\n<TR>\n<TD colspan=8>\n");
